...
Company Number: 16952633`;
  ...
  const handleVehicleRegSubmit = async () => {
    ...
    try {
      ...
      if (docResponse.ok) {
        ...
        invoiceNumber = docData.document_number;
      }
      ...
      if (clientResponse.ok) {
        ...
        if (clientData.client) {
          ...
          // No client found, just set vehicle reg and invoice number
          setFormData(prev => ({
            ...prev,
            vehicle_reg: upperReg,
            invoice_number: invoiceNumber
        ...
        // API failed, just set vehicle reg and invoice number
        setFormData(prev => ({
          ...prev,
          vehicle_reg: upperReg,
          invoice_number: invoiceNumber
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          // Inherit business details from estimate if set
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          business_name: invoice.business_name || defaultBusinessDetails.business_name,
          business_email: invoice.business_email || defaultBusinessDetails.business_email,
          business_address: invoice.business_address || defaultBusinessDetails.business_address,
          business_phone: invoice.business_phone || defaultBusinessDetails.business_phone,
          business_website: invoice.business_website || defaultBusinessDetails.business_website,
          business_workshop_location: invoice.business_workshop_location || defaultBusinessDetails.business_workshop_location
        });

        // Show business details section if any were customized
        if (invoice.business_name || invoice.business_email || invoice.business_address ||
            invoice.business_phone || invoice.business_website || invoice.business_workshop_location) {
  ...
  const fetchClientsForSelector = async (search?: string) => {
    try {
      const token = localStorage.getItem('autow_token');
      const url = search
        ? `/api/autow/client/list?search=${encodeURIComponent(search)}`
        : '/api/autow/client/list?limit=20';
  ...
  return (
    <div style={styles.container}>
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.vehicleRegModal}>
            ...
            <p style={styles.vehicleRegDescription}>
              Enter the vehicle registration to auto-generate a sequential invoice number and auto-fill customer details if found.
            </p>
      ...
      {showClientSelector && (
        <div style={styles.clientSelectorOverlay} onClick={() => setShowClientSelector(false)}>
          <div style={styles.clientSelectorModal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.clientSelectorHeader}>
              <h3 style={styles.clientSelectorTitle}>Select Client</h3>
              <button
                onClick={() => setShowClientSelector(false)}
                style={styles.clientSelectorClose}
              >
                ✕
              </button>
            </div>

            <div style={styles.clientSelectorSearch}>
              <input
                type="text"
                placeholder="Search clients..."
                value={clientSearchQuery}
                onChange={(e) => handleClientSearch(e.target.value)}
                style={styles.clientSelectorSearchInput}
                autoFocus
              />
            </div>

            <div style={styles.clientSelectorList}>
              {matchingClients.length === 0 ? (
                <div style={styles.clientSelectorEmpty}>
                  {clientSearchQuery ? 'No clients found' : 'Start typing to search...'}
                </div>
              ) : (
                matchingClients.map(client => (
                  ...
                  >
                    <div style={styles.clientSelectorItemName}>{client.name}</div>
                    <div style={styles.clientSelectorItemInfo}>
                      {client.phone && <span>{client.phone}</span>}
                      {client.vehicle_reg && <span style={styles.clientSelectorItemReg}>{client.vehicle_reg}</span>}
                    </div>
                  </button>
                ))
              )}
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  clientSelectorItemName: {
    color: '#fff',
    fontSize: '15px',
    fontWeight: '600' as const,
    marginBottom: '4px',
  },
  clientSelectorItemInfo: {
    display: 'flex',
    gap: '10px',
    fontSize: '13px',
    color: '#aaa',
